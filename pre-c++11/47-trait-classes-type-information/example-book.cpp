...
//	Categories of iterators: input_iterator, output_iterator, forward_iterator, bidirectional_iterator, random_access_iterator
//	{{{
...
//	}}}
//	Each iterator category has a 'tag struct' in the standard library: add '_tag' to the name of each, eg: input_iterator_tag
//	{{{
//	<(these are defined in std:: ?)>
...
//	Example: declare our own 'std::advance()', moves a specified iterator a specified distance
...
//	Ongoing: 2022-02-14T01:16:16AEDT (distinguish, what we can/must/should/may do, and what C++ does)
...
public:
	class iterator {
		typedef random_access_iterator_tag iterator_category;		//	(insufficent?)
		typedef T value_type;										//	(insufficent?)
	};
};
...
public:
	class iterator {
		typedef bidirectional_iterator_tag iterator_category;		//	(insufficent?)
		typedef T value_type;										//	(insufficent?)
	};
};
//	Ongoing: 2022-02-14T01:14:11AEDT 
...
int main()
{
	...
	std::iterator_traits<deque<int>::iterator>::value_type x;
